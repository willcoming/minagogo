...
4. 逐一開啟 Google Maps 連結，抓 Google 評分與評論數；若專案需要地圖座標，優先使用本地非 API 流程展開既有 Google Maps/Share 短連結並從最終 URL 抽取座標，不使用 Google Places API。
...
本專案的全頻道更新流程會在抓取後自動執行非 API 定位解析：

```bash
npm run update:channels
```

如果只想補既有資料的座標，不重新抓 YouTube，使用：

```bash
npm run resolve:maps -- --limit=all
```

這個定位流程只展開既有地圖連結並解析 URL 內的座標；不呼叫 Google Places API。搜尋型連結或沒有座標的頁面會保留為未定位。
